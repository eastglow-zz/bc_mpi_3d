     ...
     & gpid, description)
      ...
      fnameout = trim(opath)//'/'//trim(fnamebase)//'_'//trim(ts2s)//
     & '_'//trim(pid2s)//'.raw'
     ...
     & gpid, pidx, pidy, pidz, il, iu, jl, ju, kl, ku, 'fortran'
      ...
      subroutine output_i4_pvtr(a, ng, fnamebase, timestep, gpid,
     & description)
      ...
      fnameg = trim(opath)//'/'//trim(fnamebase)//'_'//
     & trim(ts2s)//'.pvtr'
     ...
     &                        '" Source="'//trim(fnamel)//'">'
      ...
      fnamel=trim(opath)//'/'//trim(fnamebase)//'_'//trim(ts2s)//'_'//
     & trim(pid2s)//'.vtr'
     ...
     & '<DataArray Name="x" type="Float32" format="ascii">'
      do i=itmp,iu-1
            write(1002,'(E21.15)') i*DXL
      enddo
      write(1002,'(A)')'</DataArray>'
      write(1002,'(A)')
     & '<DataArray Name="y" type="Float32" format="ascii">'
      do j=jtmp,ju-1
            write(1002,'(E21.15)') j*DYL
      enddo
      write(1002,'(A)')'</DataArray>'
      write(1002,'(A)')
     & '<DataArray Name="z" type="Float32" format="ascii">'
      do k=ktmp,ku-1
            write(1002,'(E21.15)') k*DZL
      enddo
      write(1002,'(A)')'</DataArray>'
      write(1002,'(A)')'</Coordinates>'
     ...
     & gpid, pidx, pidy, pidz, il, iu, jl, ju, kl, ku, 'fortran'
      ...
      subroutine output_r8_pvtr(a, ng, fnamebase, timestep, gpid,
     & description)
      ! Output of pvtr requires at least one ghost layer (ng>=1)
      use input
      use boundary_conditions
     ...
     & trim(fnamebase)//'" format="ascii">'
      ...
      subroutine merge_r8_raw()
      end subroutine merge_r8_raw
      end module outputs